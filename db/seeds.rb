...
ex_version_add = ["収穫祭","錬金術","劫掠","プロモ"]

# ex_version.each_with_index do |ev, index|
#     ExtendedVersion.create(id: index+1, name: ev)
# end

ex_version_add.each_with_index do |ev, index|
    ExtendedVersion.create(id: ex_version.count+index+1, name: ev)
end
...
#                   ["繁栄","繁"],["異郷","異"],["暗黒時代","暗"],["ギルド","ギ"],["夜想曲","夜"],["ルネサンス","ル"],["移動動物園","移"],["同盟","同"],["収穫祭","収"],["錬金術","錬"],["劫掠","劫"],["プロモ","プ"]]
...
ex_version_omit_add = [["収穫祭","収"],["錬金術","錬"],["劫掠","劫"],["プロモ","プ"]]


ex_version_omit_add.each do |evo|
    ev = ExtendedVersion.find_by(name: evo[0])
    ev.update(omit_word: evo[1])
end
